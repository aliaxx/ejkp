 ...
 */
class SasaranPtp extends \yii\db\ActiveRecord
{
    /**
     * {@inheritdoc}
     */
    ...
    {
        $sasaran = Yii::$app->db->createCommand("SELECT SUM(SASARAN) FROM TBSASARAN_PTP WHERE NOSIRI = '$this->NOSIRI'")->queryScalar();
        $pencapaian = Yii::$app->db->createCommand("SELECT SUM(PENCAPAIAN) FROM TBSASARAN_PTP WHERE NOSIRI = '$this->NOSIRI'")->queryScalar();
        $positif = Yii::$app->db->createCommand("SELECT SUM(JUMPOSITIF) FROM TBSASARAN_PTP WHERE NOSIRI = '$this->NOSIRI'")->queryScalar();
        $peratusan = Yii::$app->db->createCommand("SELECT (PENCAPAIAN/(SASARAN+PENCAPAIAN))*100 FROM TBSASARAN_PTP WHERE NOSIRI = '$this->NOSIRI'")->queryScalar();
