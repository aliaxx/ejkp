...
<tr id="itemRow_<?= $counter ?>">
    <td>
        <?= $form->field($sasaran, 'ID_JENISPREMIS[]')->widget(Select2::className(), [
            ...
            'options' => [
                'id' => 'sasaranptp-id_jenispremis_' . $counter,
                'placeholder' => '',
                'style' => 'height:26px; width:200px',
                'value' => isset($tmpData) ? $tmpData['ID_JENISPREMIS'] : null, //display from table sasaranptp -NOR300922
            ],
            'pluginOptions' => [
                'allowClear' => true,
            ],
    ...
    <td>
        ...
        <?= $form->field($sasaran, 'SASARAN[]')->textInput([
            ...
            'id' => 'sasaranptp-sasaran_' . $counter,
            'autocomplete' => 'off',
    ...
    <td>
        <?= $form->field($sasaran, 'PENCAPAIAN[]')->textInput([
            'type' => 'number', 
            'min' => ($sasaran->isNewRecord ? 1 : 0),            
            'id' => 'sasaranptp-pencapaian_' . $counter,
            'autocomplete' => 'off',
            'style' => 'height:35px; width:200px',
    ...
    <td>
        <?= $form->field($sasaran, 'JUMPOSITIF[]')->textInput([
            'type' => 'number', 
            'min' => ($sasaran->isNewRecord ? 1 : 0),            
            'id' => 'sasaranptp-jumpositif_' . $counter,
            'autocomplete' => 'off',
            'style' => 'height:35px; width:200px',
...
<?php
$this->registerJs("
$('.field-sasaranptp-id_jenispremis').toggleClass('required');
$('#sasaranptp-form').yiiActiveForm('add', {
    id: 'sasaranptp-id_jenispremis_$counter',
    name: 'SasaranPtp[id_jenispremis][]',
    container: '.field-sasaranptp-id_jenispremis_$counter',
    input: '#sasaranptp-id_jenispremis_$counter',
    error: '.help-block',
...
$('.field-sasaranptp-sasaran').toggleClass('required');
$('#sasaranptp-form').yiiActiveForm('add', {
    id: 'sasaranptp-sasaran_$counter',
    name: 'SasaranPtp[SASARAN][]',
    container: '.field-sasaranptp-sasaran_$counter',
    input: '#sasaranptp-sasaran_$counter',
    error: '.help-block',
    validate:  function (attribute, value, messages, deferred) {
...
$('.field-sasaranptp-pencapaian').toggleClass('required');
$('#sasaranptp-form').yiiActiveForm('add', {
    id: 'sasaranptp-pencapaian_$counter',
    name: 'SasaranPtp[PENCAPAIAN][]',
    container: '.field-sasaranptp-pencapaian_$counter',
    input: '#sasaranptp-pencapaian_$counter',
    error: '.help-block',
    validate:  function (attribute, value, messages, deferred) {
...
$('.field-sasaranptp-jumpositif').toggleClass('required');
$('#sasaranptp-form').yiiActiveForm('add', {
    id: 'sasaranptp-jumpositif_$counter',
    name: 'SasaranPtp[JUMPOSITIF][]',
    container: '.field-sasaranptp-jumpositif_$counter',
    input: '#sasaranptp-jumpositif_$counter',
    error: '.help-block',
    // validate:  function (attribute, value, messages, deferred) {
    //     yii.validation.required(value, messages, {message: 'Pencapaian Tidak Boleh Dibiarkan Kosong'});
    // }
