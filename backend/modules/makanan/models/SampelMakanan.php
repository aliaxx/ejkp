...
class SampelMakanan extends \yii\db\ActiveRecord
{
    ...
    public function getAttachments()
    {
        ...
        if(($this->ID == null)){
            $id = SampelMakanan::find()->max('ID'); //get last ID
            $id = ($id + 1);   
            ...
            $id = $this->ID;
            // $filesToSearch = $this->NOSIRI . '_' . $id .'*';
            // $models = SampelMakanan::findOne(['NOSIRI' => $this->NOSIRI, 'ID' => $this->ID]); 
            // $id =  $models->ID;
        }

        // $models = SampelMakanan::findOne(['NOSIRI' => $this->NOSIRI, 'ID' => $this->ID]); //display by id
        // $id =  $models->ID;
    ...
    public function saveAttachment() //amend by NOR18102022
    {
        if ($this->files) {
            ...
            if(($this->ID == null)){
                ...
                $currid = SampelMakanan::find()->max('ID'); //get last ID
                $this->ID = ($currid + 1);   
                $id = $this->ID;
            }else{
                //if id is not null, assign and use current id
    ...
    public function getSampels(){
        ...
        // $sampels = (new \yii\db\Query())->select([])->from('TBSAMPEL_SM')
        //         ->where(['NOSIRI' => $this->NOSIRI])
        //         ->limit(3)
        //         ->all();
        return $this->hasMany(SampelMakanan::className(), ['NOSIRI' => 'NOSIRI'])->orderBy(['ID' => SORT_ASC])->limit(3);
        // return $this->hasMany(SampelMakanan::className(), ['NOSIRI' => 'NOSIRI'])->orderBy(['ID' => SORT_ASC]);
