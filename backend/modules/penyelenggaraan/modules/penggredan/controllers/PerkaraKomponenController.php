...
use backend\modules\penyelenggaraan\modules\penggredan\models\PerkaraKomponenSearch;

/**
 * ButirKesalahanController implements the CRUD actions for PerkaraKomponen model.
 */
class PerkaraKomponenController extends Controller
{
    /**
     * {@inheritdoc}
     */
    public function behaviors()
    {
        return [
            'access' => [
                ...
                'rules' => [
                    [
                        'allow' => Yii::$app->access->can('komponen-read'),
                        'actions' => ['index', 'view'],
                        'roles' => ['@'],
                    ],
                    [
                        'allow' => Yii::$app->access->can('komponen-write'),
    ...
    {
        $searchModel = new PerkaraKomponenSearch();
            ...
            $actionHandler = new ActionHandler($searchModel, [
                'export-csv' => function ($model) use ($dataProvider) {
                    ...
                    foreach ($models as $model) {
                        $rows[] = [
                            $model->JENIS, $model->kesalahan->seksyen, $model->KODKOMPONEN, $model->PRGN,
                    ...
                    header('Content-Disposition: attachment; filename='.Yii::$app->controller->id.'PerkaraKomponen.csv');
                ...
                'export-pdf' => function ($model) use ($dataProvider) {
                    ...
                    $mpdf->Output(Yii::$app->controller->id.'PerkaraKomponen.pdf', \Mpdf\Output\Destination::DOWNLOAD);
                },
            ]);
     ...
     * @return mixed
     */
    public function actionView($JENIS, $KODPERKARA, $KODKOMPONEN)
    {
        $model = $this->findModel($JENIS, $KODPERKARA, $KODKOMPONEN);
     ...
     * @return mixed
     */
    public function actionUpdate($JENIS, $KODPERKARA, $KODKOMPONEN)
    {
        $model = $this->findModel($JENIS, $KODPERKARA, $KODKOMPONEN);
     ...
     * @return mixed
     */
    public function actionDelete($JENIS, $KODPERKARA, $KODKOMPONEN)
    {
        $model = $this->findModel($JENIS, $KODPERKARA, $KODKOMPONEN);
    ...
    /**
     * Finds the PerkaraKomponen model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param string $JENIS
     * @param integer $KODPERKARA
     * @param integer $KODKOMPONEN
     * @return PerkaraKomponen the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($JENIS, $KODPERKARA, $KODKOMPONEN)
    {
        if (($model = PerkaraKomponen::findOne(['JENIS' => $JENIS, 'KODPERKARA' => $KODPERKARA, 'KODKOMPONEN' => $KODKOMPONEN])) !== null) {
            return $model;
        } else {
