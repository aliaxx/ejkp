...
/* @var $searchModel backend\modules\penyelenggaraan\modules\subunit\models\SubunitSearch */
...
$source = \backend\modules\penyelenggaraan\models\ParamDetail::find()->where(['STATUS' => 1])->andWhere(['KODJENIS' => 24])->orderBy(['PRGN' => SORT_ASC])->all();
$option['ID_KODUNIT'] = \yii\helpers\ArrayHelper::map($source, 'KODDETAIL', 'PRGN'); //ID_KODUNIT is options, KODDETAIL is column where we want to get data --NOR180822
?>
<div class="subunit-index">

    <?php $form = ActiveForm::begin(); ?>
        <?= \codetitan\widgets\ActionBar::widget([
            'target' => 'primary-grid',
            'permissions' => ['new' => Yii::$app->access->can('subunit-write')],
        ]) 
        ?>
    ...
    <?php $output = GridView::widget([
        ...
        'columns' => [
            ...
            [
                ...
                'format' => 'raw',
                'filter' => $option['ID_KODUNIT'], //dropdown filter in field search gridview --NOR180822
                'value' => function($model) {
                    // this to display prgn of koddetail from tbparameterdetail --NOR180822
            ...
            [
                ...
                'visibleButtons' => [
                    'view' =>   \Yii::$app->access->can('subunit-read'),
                    'update' => \Yii::$app->access->can('subunit-write'),
                    'active' => function ($model) {
                        return \Yii::$app->access->can('subunit-write') && ($model->STATUS == OptionHandler::STATUS_TIDAK_AKTIF);
                    },
                    'inactive' => function ($model) {
                        return \Yii::$app->access->can('subunit-write') && ($model->STATUS == OptionHandler::STATUS_AKTIF);
                    },
                ],
            ],       
        ],
