...
//dropdown search dekat grid table untuk KODAKTA. 
$source = \backend\modules\penyelenggaraan\models\Akta::find()->where(['STATUS' => 1])->orderBy(['PRGN' => SORT_ASC])->all();
$option['KODAKTA'] = \yii\helpers\ArrayHelper::map($source, 'KODAKTA', 'PRGN');
...
<div>
    ...
    <?php $form = ActiveForm::begin(); ?>
        <?= \codetitan\widgets\ActionBar::widget([
            'target' => 'primary-grid',
            'permissions' => ['new' => Yii::$app->access->can('seksyen-kesalahan-write')],
        ]) ?>
    ...
    <?php $output = GridView::widget([
        ...
        'columns' => [
            ...
            [
                'attribute' => 'KODAKTA',       
                'value' => 'akta.PRGN', //Display nama jenis instead of Kod Jenis 
                'filter' => $option['KODAKTA'], //dropdown search untuk display data bila pilih kod jenis yang didaftarkan di ParamHeader. Nurul 250722
            ],
            'KODSALAH',
            'SEKSYEN',
            'PRGNSEKSYEN',
            ...
            [
                ...
                'buttons' => [
                    'active' => function ($url, $model) {
                        ...
                        return Html::a('<span class="fa fa-toggle-off"></span>', ['delete', 'KODAKTA' => $model->KODAKTA, 'KODSALAH' => $model->KODSALAH], $options);
                    },
                    'inactive' => function ($url, $model) {
                        ...
                        return Html::a('<span class="fa fa-toggle-on"></span>', ['delete', 'KODAKTA' => $model->KODAKTA, 'KODSALAH' => $model->KODSALAH], $options);
                    },
                ],
                'visibleButtons' => [
                    'view' =>   \Yii::$app->access->can('seksyen-kesalahan-read'),
                    'update' => \Yii::$app->access->can('seksyen-kesalahan-write'),
                    'active' => function ($model) {
                        return \Yii::$app->access->can('seksyen-kesalahan-write') && ($model->STATUS == OptionHandler::STATUS_TIDAK_AKTIF);
                    },
                    'inactive' => function ($model) {
                        return \Yii::$app->access->can('seksyen-kesalahan-write') && ($model->STATUS == OptionHandler::STATUS_AKTIF);
                    },
                ],
            ],
        ],
