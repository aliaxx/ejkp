...
$data['KODAKTA'] = [''];
$source = Yii::$app->db->createCommand("SELECT * FROM TBAKTA")->queryAll();
$data['KODAKTA'] = ArrayHelper::map($source, 'KODAKTA', 'PRGN');
...
<div>
    <?php $form = ActiveForm::begin([
        ...
        <?= $form->field($model, 'KODSALAH')->textInput(['disabled' => !$model->isNewRecord]) ?>
        <?= $form->field($model, 'SEKSYEN') ?>
        <?= $form->field($model, 'PRGNSEKSYEN')->textArea(['maxlength' => true, 'style' => 'height:140px']) ?>
        <?= $form->field($model, 'PRGN1') ?>
        <?= $form->field($model, 'PRGN2') ?>
        <?= $form->field($model, 'PRGNDENDA')->textArea(['maxlength' => true, 'style' => 'height:140px']) ?>
