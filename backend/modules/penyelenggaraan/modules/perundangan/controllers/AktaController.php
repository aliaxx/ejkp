...
use backend\modules\penyelenggaraan\modules\perundangan\models\AktaSearch;

/**
 * AktaController implements the CRUD actions for Akta model.
 */
class AktaController extends Controller
{
    ...
    public function behaviors()
    {
        return [
            'access' => [
                ...
                'rules' => [
                    [
                        'allow' => Yii::$app->access->can('akta-read'),
                        'actions' => ['index', 'view'],
                        'roles' => ['@'],
                    ],
                    [
                        'allow' => Yii::$app->access->can('akta-write'),
            ...
            $actionHandler = new ActionHandler($searchModel, [
                'export-csv' => function ($model) use ($dataProvider) {
                    ...
                    foreach ($models as $model) {
                        $rows[] = [
                            $model->KODAKTA, $model->PRGN, OptionHandler::resolve('STATUS', $model->STATUS),
                            $model->updatedByUser->NAMA, date('Y-m-d H:i:s', $model->TRKHAKHIR),
                        ];
                    }

                    header('Content-Type: text/csv; charset=utf-8');
                    header('Content-Disposition: attachment; filename='.Yii::$app->controller->id.'Akta.csv');
                ...
                'export-pdf' => function ($model) use ($dataProvider) {
                    ...
                    $mpdf->Output(Yii::$app->controller->id.'Akta.pdf', \Mpdf\Output\Destination::DOWNLOAD);
                },
            ]);
    ...
    /**
     * Displays a single KODAKTA model.
     * @param integer $id
     * @return mixed
     */
    ...
    /**
     * Updates an existing Akta model.
    ...
    /**
     * Deletes an existing Akta model.
    ...
    /**
     * Finds the Akta model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return Akta the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id)
    {
        if (($model = Akta::findOne($id)) !== null) {
            return $model;
        } else {
